...
// 助詞「を」の表記ゆれ。カタカナ主体で認識されると「オ」「ヲ」に化ける。
//   実測 2026-08-05:「かぎかっことじを再変換」→『カギカッコトジオ再変換』
// これを許さないと、記号語のように誤認識されやすい語ほどコマンドで直せない
// （直したい語ほど命令が通らない）という逆転が起きる。末尾の命令語が一致する
// ことが条件なので、本文の「オ」を巻き込む心配はない。
const WO = "[をオヲお]";
...
const RECONVERT_TARGET_RE = new RegExp(
...
const RESPEAK_TARGET_RE = new RegExp(
