...
var RESPEAK_EXPLICIT_WORDS = words(
  ["\u8A00\u3044\u76F4\u3057", "\u3044\u3044\u306A\u304A\u3057"],
  ["\u8A00\u3044\u76F4\u3057\u3066", "\u3044\u3044\u306A\u304A\u3057\u3066"],
  ["\u8A00\u3044\u76F4\u3059", "\u3044\u3044\u306A\u304A\u3059"],
  ["\u3053\u3053\u3092\u8A00\u3044\u76F4\u3057", "\u3053\u3053\u3092\u3044\u3044\u306A\u304A\u3057"],
...
var RESPEAK_EXPLICIT = RESPEAK_EXPLICIT_WORDS.map((w) => w.word);
var RESPEAK_PLAIN = RESPEAK_PLAIN_WORDS.map((w) => w.word);
...
function parseCommand(rawText, prefix = "") {
  ...
  if (RESPEAK_EXPLICIT.includes(text))
    return { kind: "respeak", explicit: true };
  if (RESPEAK_PLAIN.includes(text))
    return { kind: "respeak", explicit: false };
...
var VoxCraftSettingTab = class extends import_obsidian3.PluginSettingTab {
  ...
  display() {
    ...
    new import_obsidian3.Setting(containerEl).setName("\u6587\u5B57\u8D77\u3053\u3057\u306E\u8A00\u8A9E").setDesc(
      "\u53E3\u8FF0\u306B\u306F\u5F71\u97FF\u3057\u306A\u3044\uFF08\u5E38\u306B\u65E5\u672C\u8A9E\uFF09\u3002\u65E2\u5B9A\u306E\u300C\u81EA\u52D5\u5224\u5225\u300D\u306F\u3001\u30C1\u30E3\u30F3\u30AF\u3054\u3068\u306B\u8A00\u8A9E\u3092\u5224\u5B9A\u3057\u3066\u5207\u308A\u66FF\u3048\u308B\u3002\u7D14\u65E5\u672C\u8A9E\u306E\u9332\u97F3\u3067\u8AA4\u3063\u3066\u82F1\u8A9E\u3068\u5224\u5B9A\u3057\u305F\u306E\u306F\u5B9F\u6E2C 0.4\u301C0.8%\uFF08\u5168\u90E81.6\u79D2\u672A\u6E80\u306E\u77ED\u3044\u30C1\u30E3\u30F3\u30AF\u3067\u3001\u305D\u308C\u3089\u306F\u5224\u5B9A\u304B\u3089\u5916\u3057\u3066\u3042\u308B\uFF09\u3002\u4EE3\u511F\u306F1\u30C1\u30E3\u30F3\u30AF\u3042\u305F\u308A\u7D0496\u30DF\u30EA\u79D2\u3002\u56FA\u5B9A\u3057\u305F\u3044\u3068\u304D\u3060\u3051\u65E5\u672C\u8A9E\uFF0F\u82F1\u8A9E\u3092\u9078\u3076\u3002\u82F1\u8A9E\u3067\u306F\u65E5\u672C\u8A9E\u5411\u3051\u306E\u5F8C\u51E6\u7406\uFF08\u8F9E\u66F8\u30FB\u81EA\u52D5\u53E5\u8AAD\u70B9\uFF09\u304C\u5F53\u305F\u3089\u306A\u3044\u3002"
    ).addDropdown((d) => {
...
function classify(ch) {
  if (/[一-鿿㐀-䶿々〇]/u.test(ch))
    return "han";
  if (/[ぁ-ゖゝゞ]/u.test(ch))
    return "hiragana";
  if (/[ァ-ヺーヽヾｦ-ﾟ]/u.test(ch))
    return "katakana";
  if (/[0-9A-Za-z０-９Ａ-Ｚａ-ｚ]/u.test(ch))
    return "latin";
  return "none";
}
function runAt(text, pos) {
  const cls = classify(text[pos]);
  if (cls === "none")
    return null;
  let from = pos;
  while (from > 0 && classify(text[from - 1]) === cls)
    from -= 1;
  let to = pos + 1;
  while (to < text.length && classify(text[to]) === cls)
    to += 1;
  return { from, to, cls };
}
function wordRangeAt(text, pos) {
  ...
  if (run.cls === "han") {
    let end = to;
    while (end < text.length && classify(text[end]) === "hiragana")
      end += 1;
    if (end - to <= OKURIGANA_MAX)
      to = end;
  } else if (run.cls === "hiragana" && to - from <= OKURIGANA_MAX) {
    let start = from;
    while (start > 0 && classify(text[start - 1]) === "han")
      start -= 1;
    if (start < from)
      from = start;
  }
...
var DictationToolbar = class {
  ...
  show() {
    ...
    this.wordBtn(
      bar,
      "\u8A00\u3044\u76F4\u3057",
      "\u30AB\u30FC\u30BD\u30EB\u4F4D\u7F6E\u306E\u8A9E\uFF08\u307E\u305F\u306F\u9078\u629E\u7BC4\u56F2\uFF09\u3092\u8A00\u3044\u76F4\u3059 \u2014 \u6B21\u306E\u767A\u8A71\u3067\u7F6E\u304D\u63DB\u3048",
      () => this.cb.onRespeak()
    );
    this.wordBtn(
      bar,
      "\u518D\u5909\u63DB",
      "\u30AB\u30FC\u30BD\u30EB\u4F4D\u7F6E\u306E\u8A9E\uFF08\u307E\u305F\u306F\u9078\u629E\u7BC4\u56F2\uFF09\u3092\u518D\u5909\u63DB \u2014 \u5019\u88DC\u304B\u3089\u9078\u3076",
      () => this.cb.onReconvert()
    );
...
\u300C3\u756A\u300D\u3067\u9078\u629E\u3001\u300C\u78BA\u5B9A\u300D/\u300C\u30AD\u30E3\u30F3\u30BB\u30EB\u300D\u3067\u9589\u3058\u307E\u3059\u3002`
  ...
  runCommand(cmd) {
    switch (cmd.kind) {
      ...
      case "reconvertSelection":
        ...
        return true;
      case "respeak":
        if (!cmd.explicit && !this.hasSelection())
          return false;
        this.startRespeak();
        return true;
      case "respeakTarget":
  ...
  async reconvertSelection() {
    ...
    if (!range) {
      new import_obsidian7.Notice(
        "VoxCraft: \u518D\u5909\u63DB\u3059\u308B\u5834\u6240\u304C\u6C7A\u307E\u308A\u307E\u305B\u3093\u3002\u76F4\u3057\u305F\u3044\u8A9E\u306E\u4E2D\u306B\u30AB\u30FC\u30BD\u30EB\u3092\u7F6E\u304F\u304B\u3001\u7BC4\u56F2\u3092\u9078\u629E\u3057\u3066\u304F\u3060\u3055\u3044\u3002"
      );
      return;
    }
  ...
  startRespeak() {
    ...
    if (!range) {
      new import_obsidian7.Notice(
        "VoxCraft: \u8A00\u3044\u76F4\u3059\u5834\u6240\u304C\u6C7A\u307E\u308A\u307E\u305B\u3093\u3002\u76F4\u3057\u305F\u3044\u8A9E\u306E\u4E2D\u306B\u30AB\u30FC\u30BD\u30EB\u3092\u7F6E\u304F\u304B\u3001\u7BC4\u56F2\u3092\u9078\u629E\u3057\u3066\u304F\u3060\u3055\u3044\u3002"
      );
      return;
    }
    const text = cm.state.doc.sliceString(range.from, range.to);
