...
class Config:
    ...
    # これより短いチャンクは言語を判定しない（直前のチャンクの言語を引き継ぐ）。
    # 実測 2026-08-08: 純日本語の録音2本（64分/34分・計487チャンク）で誤って en と
    # 判定したのは3件（0.4%と0.8%）だけで、**3件とも1.6秒未満**、うち1件は日本語認識も
    # 空の無音だった。短いチャンクを判定から外せば、誤判定は実質ゼロになる。
    ...
    #
    # 2026-08-06 まで 4.0 / 2.0 / 2.0 だった。この値では記者会見で連結がほぼ成立しない。
    # 会見は「間」だらけで、break 2.0秒（これ以上空いたら繋がない）と hold 2.0秒
    # （待つのは実時間で2秒まで）の両方に毎回引っかかるため。
    #
    # 実測（NTT決算会見 36.8分・会場の音をPCのマイクで収録）:
    #     4.0/2.0/2.0 : 連結後 634件 中央3.2秒 4秒未満 64.4%
    #     10/6/4      : 連結後 263件 中央8.2秒 4秒未満  9.9%
    #   捨てられた53チャンクのうち47件が4秒未満 ＝ 連結が救うはずだった長さ。
    #   同じ10分・同じ turbo で 定型句幻覚 15件 → 0件、認識 511秒 → 116秒。
    #
    # **長い連結が不利になる録音は見つからなかった。** 近接マイクの取材（6.8分）でも
    # 本文 2,869 → 2,864字（-0.2%）、段落 9 → 8、破棄 0件のまま、認識は 195 → 79秒。
    # チャンクが減ると速くなるのは、Whisper が入力長によらず30秒窓を計算するため。
    # そこで 2026-08-06 に既定を入れ替えた。短い連結は下の nearby_* に残してある。
    #
    # **モデル差し替えでは直らない。** 同条件の large-v3 は 15件→20件と悪化した。
    # 3秒の音声を30秒窓に入れれば残りは無音パディングで、そこに書くべきものは
    # 音の中に無い。大きいモデルほど学習データの事前分布を堂々と出す。
    transcribe_join_sec: float = float(_env("VOXCRAFT_JOIN_SEC", "10.0"))
    # ただし実時間でこれ以上は次を待たない（孤立した短い発話を画面に出さないため）。
    # 表示の遅れはこの秒数が上限になる。
    transcribe_join_hold_sec: float = float(_env("VOXCRAFT_JOIN_HOLD_SEC", "6.0"))
    # これ以上の息継ぎをまたぐ連結はしない。繋いでしまうと「話の切れ目」が
    # チャンクの内側に埋もれ、段落分けの材料が消えるため。
    # 実測: 新たにまたぐことになる「2〜4秒の息継ぎ」は会見36.8分で41件（全体の3.3%）、
    # 近接マイクの取材で1件（1.1%）。段落分けは悪化しなかった（会見の5分で 7→8段落）。
    transcribe_join_break_sec: float = float(_env("VOXCRAFT_JOIN_BREAK_SEC", "4.0"))

    # --- 対面インタビュー（低遅延）プロファイル ---
    # 目の前の相手と話していて、テキストが早く出てほしいときだけ使う。
    # 上の既定（10/6/4）はテキストが8秒ほどのまとまりで出るのに対し、こちらは
    # 3〜4秒おきに出る。代わりに短いチャンクが単体で Whisper に渡る機会が増える
    # ＝ 定型句の幻覚が出やすくなるので、会議・会見・動画には使わないこと。
    nearby_join_sec: float = float(_env("VOXCRAFT_NEARBY_JOIN_SEC", "4.0"))
    nearby_join_hold_sec: float = float(_env("VOXCRAFT_NEARBY_JOIN_HOLD_SEC", "2.0"))
    nearby_join_break_sec: float = float(_env("VOXCRAFT_NEARBY_JOIN_BREAK_SEC", "2.0"))
