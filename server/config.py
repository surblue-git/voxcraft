...
class Config:
    ...
    # --- 遠いマイク（会見・発表会）プロファイル ---
    # 会場のスピーカーを離れた場所のマイクで拾う録音では、上の既定値では連結が
    # ほとんど成立しない。記者会見は「間」だらけで、break 2.0秒（これ以上空いたら
    # 繋がない）と hold 2.0秒（待つのは実時間で2秒まで）の両方に毎回引っかかるため。
    #
    # 実測 2026-08-06（NTT決算会見 36.8分・会場の音をPCのマイクで収録）:
    #   VADが拾ったのは音声の 96.2%。定型句の幻覚に関する限り、壊れているのは連結。
    #   （残り3.8%＝約84秒は無音ではなく、12秒まるごと落ちた実発話。これは別の
    #    原因＝エネルギーVADの絶対しきい値。vad.py の _EnergyDetector を参照）
    #     既定 (4.0/2.0/2.0) : 連結後 634件 中央3.2秒 4秒未満 64.4%
    #     この値 (10/6/4)    : 連結後 263件 中央8.2秒 4秒未満  9.9%
    #   捨てられた53チャンクのうち47件が4秒未満 ＝ 連結が救うはずだった長さ。
    #
    # 同じ10分・同じ turbo での定型句幻覚（server.log の実運用は17件）:
    #     既定    15件 / logprob -0.568 / 認識 511秒
    #     この値   0件 / logprob -0.384 / 認識 116秒
    # チャンクが減ると速くもなる。Whisper は入力長によらず30秒窓を計算するため。
    #
    # **モデル差し替えでは直らない。** 同条件の large-v3 は 15件→20件と悪化した。
    # 3秒の音声を30秒窓に入れれば残りは無音パディングで、そこに書くべきものは
    # 音の中に無い。大きいモデルほど学習データの事前分布を堂々と出す。
    #
    # VAD側（silence_sec 0.35→0.8）も併せて試したが、幻覚 1件 / logprob -0.372 と
    # 連結だけの場合と差が無かった。音の切り方には触れないほうが副作用が少ない。
    far_mic_join_sec: float = float(_env("VOXCRAFT_FAR_MIC_JOIN_SEC", "10.0"))
    far_mic_join_hold_sec: float = float(_env("VOXCRAFT_FAR_MIC_JOIN_HOLD_SEC", "6.0"))
    far_mic_join_break_sec: float = float(_env("VOXCRAFT_FAR_MIC_JOIN_BREAK_SEC", "4.0"))
